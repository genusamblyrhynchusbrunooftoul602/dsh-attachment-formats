...
/**
 * 命中检查：manifest 存在、schema 版本一致、sourceHash 与源文件一致，
 * 且主文本文件在盘上。命中返回 { manifest, text, docFile }，否则 null。
 */
export async function readCachedTextIfValid(root, id, sourceHash) {
  if (!/^[a-f0-9]{8}$/i.test(String(id ?? ""))) return null;
  const dir = join(root, id);
  try {
...
/** 尽力而为的过期清理：删除 root 下「创建/最后访问超过 7 天」的子目录。 */
...
export async function readCachedDoc(cwd, id) {
  ...
  const text = await readFile(join(dir, docFile), "utf8");
  void touchCachedDoc(root, id); // 访问即续期（TTL 语义：创建/最后访问 7 天）
...
export async function removeCachedDocs(cwd, ids) {
  ...
  const removed = [];
  for (const id of ids) {
    if (!/^[a-f0-9]{8}$/i.test(String(id ?? ""))) continue;
    try {
