...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function useBusState() {
			return useSyncExternalStore(subscribeBus, () => busState);
		}
		...
		function setChips(items) {
			chipsState = { sessionId: activeSession?.sessionId, items };
		...
		let chipSeq = 0;
		function addChips(entries) {
			const current = chipsState.sessionId === activeSession?.sessionId ? chipsState.items : [];
			const next = [...current];
		...
		// ---- host conversion -----------------------------------------------
		/** 工作区零拷贝：按「文件名 + 字节数」询问主机是否有同源文件（P2-1）。 */
		async function resolveWorkspaceRef(file, cwd) {
			if (cwd === undefined) return null;
			try {
				const params = new URLSearchParams({ cwd, name: file.name, size: String(file.size) });
		...
		function mergeChipsIntoDraft() {
			const mine = chipsState.sessionId === activeSession?.sessionId ? chipsState.items : [];
			if (mine.length === 0) return;
			const el = composerTextarea();
			if (el === null || el.disabled || el.readOnly) return; // 忙/锁定：不合并，卡片保留
			injectTexts(mine.map((item) => ({
				name: item.name,
				text: item.text,
				note: item.kind === "note",
		...
		// ---- intake pipeline --------------------------------------------------
		// 当前活动会话（由 input.left 插槽渲染时刷新；拖放/粘贴/按钮共用）。
		let activeSession = { sessionId: undefined, cwd: undefined, sessionsService: undefined };
		function currentCwd() {
			const { sessionId, sessionsService } = activeSession;
			if (sessionsService === undefined || sessionId === undefined) return undefined;
			try {
				return sessionsService.list.getSnapshot().byId[sessionId]?.cwd ?? undefined;
			} catch {
				return undefined;
			}
		}
		...
		async function intake(files) {
			...
			for (const file of files) {
				...
				try {
					switch (kind) {
						...
						case "text": {
							// 工作区零拷贝（P2-1）：较大文本文件先尝试按名+大小解析同源路径，
							// 命中则挂「引用」卡片，不读内容、不上传字节，模型用 read 工具读取。
							if (file.size > 512 * 1024) {
								setBus({ phase: "working", label: file.name, detail: "正在解析工作区路径…" });
								const ref = await resolveWorkspaceRef(file, cwd);
								if (ref !== null) {
									chips.push({
										name: file.name,
										kind: "ref",
							...
							if (file.size > MAX_CACHE_BYTES) {
								throw new Error(`文件过大（超过 ${Math.round(MAX_CACHE_BYTES / 1024 / 1024)}MB），且未在工作区找到同源文件（未附加）`);
							}
							...
							if (cached.kind !== "index") throw new Error("长文本转存失败");
							chips.push({
								name: file.name,
								kind: "card",
								text: cached.card,
			...
			if (images.length > 0) redispatchDrop(images);
			if (chips.length > 0) addChips(chips);
			const parts = [];
		...
		function CacheSettings() {
			...
			const refresh = useCallback(async () => {
				...
				try {
					const cwd = currentCwd();
					const url = `/api/attach-formats/cache${cwd === undefined ? "" : `?cwd=${encodeURIComponent(cwd)}`}`;
			...
			const act = useCallback(async (path, body = {}) => {
				...
				try {
					const response = await fetch(path, {
						method: "POST",
						headers: { "content-type": "application/json" },
						body: JSON.stringify({ cwd: currentCwd(), ...body })
					});
					const payload = await response.json();
			...
			return jsx("div", {
				...
				children: [
								...
								children: [
									...
									jsx("div", {
										...
										children: state.loading
											? "读取中…"
											: `${state.docs.length} 个已转存文档 · 共 ${sizeText}（工作区 .dsh-attachments/，7 天未用自动清理）`
									})
								]
							}),
							jsx("button", {
					...
					state.error !== null
						? jsx("div", { className: "dshaf-settings-error", children: state.error })
						: null,
					state.docs.length === 0 && !state.loading
						? jsx("div", { className: "dshaf-settings-empty", children: "还没有转存的文档——拖入超过 8 万字符的文本或长 PDF 后会出现在这里。" })
						: jsx("div", {
