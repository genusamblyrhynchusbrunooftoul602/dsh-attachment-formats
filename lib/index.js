...
import {
  ...
  shortHashOf, touchCachedDoc, writeCache
} from "./cache.js";
import {
...
async function textResultToResponse(ctx, bytes, input, cwd, textResult, directLimit) {
  ...
  // 无论直插还是索引都先落盘（全量、不截断）——目录即转换缓存，命中可跳过引擎
  ...
  const { base } = await writeCache({ root, rel }, id, input, "pdf", files, {
    ...
    card: buildIndexCard({
      input,
      base: rel === null ? join(root, id) : `${rel}/${id}`,
      docFile: "doc.md",
      pageCount,
      lineCount,
      charCount,
...
/**
 * 转换缓存命中（解析引擎之前调用）：同一源文件 + 同 schema 版本 + 盘上产物
 * 齐全 → 直接用落盘文本/索引卡应答，跳过 pymupdf4llm/OCR 等昂贵引擎。
 * 未命中返回 null。
 */
export async function cachedTextResponse(ctx, bytes, input, cwd, limit) {
  const id = shortHashOf(bytes);
  const { root, rel } = resolveCacheRoot(cwd);
  const hit = await readCachedTextIfValid(root, id, id);
  ...
  const dir = rel === null ? join(root, id) : `${rel}/${id}`;
  const limitNow = limit ?? DIRECT_TEXT_CHARS;
  if (charCount <= limitNow) {
    const withNotes = Array.isArray(manifest.notes) && manifest.notes.length > 0
      ? `${text}\n\n[附件说明] ${manifest.notes.join("；")}`
      : text;
    return { input, kind: "text", text: withNotes, pageCount, charCount, engine, ocr };
  }
  return {
    input,
    kind: "index",
    card: typeof manifest.card === "string"
      ? manifest.card
      : buildIndexCard({
        input,
        base: dir,
        ...
        lineCount,
        charCount,
        outline: Array.isArray(manifest.outline) ? manifest.outline.map((entry) => ({ text: entry })) : [],
        hasPageImages: manifest.hasPageImages === true,
        notes: ocr ? ["文本来自 OCR 识别，可能有误差。"] : []
      }),
    docPath: `${dir}/${docFile}`,
    pageCount,
    lineCount,
...
async function convertPdfFile(ctx, bytes, input, cwd, directLimit) {
  ...
  const cached = await cachedTextResponse(ctx, bytes, input, cwd, directLimit);
  ...
  // ---- 引擎 3：扫描件 OCR（v0.6：百度云 API 优先 → VLM → tesseract.js 兜底）----
  if (policy.ocr !== "off") {
    try {
      const rendered = await renderPdfPages(bytes, {
    ...
    () => ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          const removed = await removeCachedDocs(cwd, Array.isArray(body?.ids) ? body.ids : []);
