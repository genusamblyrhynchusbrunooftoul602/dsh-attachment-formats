 ...
 */
async function convertOfficeFile(ctx, bytes, input, cwd, extract, directLimit, kind, cacheBytes = bytes) {
  // 缓存命中：跳过提取（重复拖入直接复用）
  ...
  try {
    switch (kind) {
      ...
      case "ppt": {
        ...
        if (cached !== null) {
          if (cached.kind === "index" && typeof cached.engine === "string" && !String(cached.engine).startsWith("libreoffice+")) {
            cached.engine = `libreoffice+${cached.engine}`;
          }
          return cached;
        }
        ...
        const result = await convertOfficeFile(ctx, converted.data, input, cwd, extract, limit, kind, bytes);
