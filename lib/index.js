...
import {
  ...
  sha256Of, shortHashOf, touchCachedDoc, writeCache, CACHE_SCHEMA_VERSION
} from "./cache.js";
import {
...
async function convertOfficeFile(ctx, bytes, input, cwd, extract, directLimit, kind) {
  // 缓存命中：跳过提取（重复拖入直接复用）
  const cached = await cachedTextResponse(ctx, bytes, input, cwd, directLimit, kind);
  if (cached !== null) return cached;
  ...
  try {
    switch (kind) {
      ...
      case "ppt": {
        ...
        const result = await convertOfficeFile(ctx, converted.data, input, cwd, extract, limit, kind);
