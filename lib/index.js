...
import { enginePolicy, probePythonEngine, runPythonPdf } from "./convert/provider.js";
import { ocrPages, OCR_PAGE_CAP } from "./convert/ocr.js";
import { cleanupCache, listCachedDocs, readCachedDoc, resolveCacheRoot, shortHashOf, writeCache } from "./cache.js";
import {
...
/** python 引擎（高保真）的页数上限：更大文档用 pdfjs 快速引擎。 */
const PYTHON_PAGE_LIMIT = 40;
...
/** PDF：文字优先，v3 引擎链 python(pymupdf4llm) → builtin(pdfjs) → OCR → 页图。 */
async function convertPdfFile(ctx, bytes, input, cwd, directLimit) {
  ...
  if (pythonAvailable) {
    ...
    if (probedPages !== null && probedPages <= PYTHON_PAGE_LIMIT) {
      ...
      textError = `python 引擎仅处理 ≤${PYTHON_PAGE_LIMIT} 页的 PDF，本文件 ${probedPages} 页改用内置引擎`;
    }
  }
  ...
  if (policy.ocr !== "off") {
    try {
      ...
      if (rendered.pages.length > 0) {
        ...
        if (
          ocr.chars >= Math.max(10, 10 * rendered.pages.length) &&
          ocr.confidence >= OCR_MIN_CONFIDENCE
        ) {
